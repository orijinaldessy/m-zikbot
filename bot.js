...
client.on("ready", () => {
  client.user.setGame(`%yardım | Arsen's Home`, "https://www.twitch.tv/dessyofficial");
        }, 
...
client.on('message', async message => {
  ...
  message.channel.send(`Bot Bilgi Kanallarının kurulumu başlatılsın mı? başlatılacak ise **evet** yazınız.`)
      if (!message.member.hasPermission('ADMINISTRATOR'))
  return message.channel.send("<a:basarisiz:596887378476400651> | Bu Kodu `Yönetici` Yetkisi Olan Kişi Kullanabilir.");
      message.channel.awaitMessages(response => response.content === 'evet', {
        max: 1,
        time: 10000,
        errors: ['time'],
...
function cpanel1() {
    return new Promise(resolve => {
        setTimeout(() => {
            client.channels.get(`612748436332347408`).setName(`register`);
            cpanel2();
        }, 10000);
      });
}
  ...
  function cpanel2() {
    return new Promise(resolve => {
        setTimeout(() => {
            client.channels.get(`612748436332347408`).setName(`kayıt-ol`);
            cpanel3();
        }, 10000);
      });
  }
  function cpanel3() {
    return new Promise(resolve => {
        setTimeout(() => {
            client.channels.get(`612748436332347408`).setName(`r-e-g-i-s-t-e-r`);
            cpanel4();
...
function cpanel4() {
    return new Promise(resolve => {
        setTimeout(() => {
            client.channels.get(`612748436332347408`).setName(`k-a-y-ı-t-o-l`);
            cpanel1();
        }, 10000); //Hızı düşürmeyin
      });
  }
