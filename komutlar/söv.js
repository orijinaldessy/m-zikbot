exports.run = async (client, msg, args) => {
 ...
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Ağzına sıçtığım sen kimsinde Alpere söversin!')
}})
  if(member.id === "481412397895122963")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Arsenime sövemezsin!!`)
}})
  if(member.id === "431772260508893194")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Alperime sövemezsin!!`)
}})
  if(member.id === client.user.id){
    msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Siktir duygusuz pezevenk bide bana sövecek!!`)
}})
  }
  else{
  msg.channel.send({embed: {
